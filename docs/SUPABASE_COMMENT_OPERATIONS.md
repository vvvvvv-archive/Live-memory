...
- `owned=true` または自分の投稿だけを返すRPC結果を使う
...
- ブラウザデータ削除後は過去投稿を本人のものとして取得できないことを明記する
...
### 完全削除してもよいCodexテストコメント候補の確認SQL

推測で削除せず、以下で候補を確認してから判断します。
...
select
  ...
  deleted_at
from public.prototype_comments
where page_key like 'codex-%'
   or page_key like 'codex:%'
   or page_key like 'codex-rpc-stage%'
   or body ilike '%codex%'
   or body ilike '%test%'
   or body ilike '%テスト%'
...
### 案2: deleted_atから30日または90日経過したコメントを定期削除
...
プレ公開直後は、まず案1の個別確認方式を推奨します。投稿数が増えてから90日定期削除へ移行するのが安全です。
